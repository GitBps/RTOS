...
int sendMessage (struct my_msgbuf *buf,int msqid, long len)
{
    ...
    debug("Client[%ld]:Buffer to send [%s], len=[%ld], msqid=[%d] \n", buf->msender,
                      buf->mtext, len, msqid);
...
int startReceive(struct my_msgbuf *buf,int msqid)
{
    ...
    debug("Client: Waiting for incoming message\n");
    ...
    buf->mtext[recvLen] = '\0';
    debug("Client: Sender [%ld], RxLen [%ld], Buffer received =[%s], msqid=[%d] \n",
...
int connectSocket()
{
    ...
    debug("Client Socket Connected port \n");
    return sockfd;
}
