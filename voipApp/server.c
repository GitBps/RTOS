...
void sendBack(int sockfd, pa_simple *);
void startMonitoring();
void verifyPerformance();
pid_t Fork(void);

#define MAX_CLIENTS 1024
#define MAX_BUFFER 4*2048 // 8K for voice
/*************************/
...
int main(int argc, char **argv)
{
 ...
 static const pa_sample_spec ss = {
    ...
    if (!(s = pa_simple_new(NULL, "myvoipapp", PA_STREAM_PLAYBACK, NULL, "playback", &ss, NULL, NULL, &error))) {
    ...
    //inet_pton(AF_INET, "192.168.43.230", &(servaddr.sin_addr));
    ...
    for ( ; ; )
    {
        ...
        if ((childpid = Fork()) == 0)
        {
            while(1)
            {
                ...
                sendBack(connfd, s);    /* process the request */
            }
            exit(0);
        }
...
void sendBack(int sockfd, pa_simple *s)
{
    ssize_t     n = 0;
    char        buf[MAX_BUFFER];
    int		error;
...
again:
    ...
    {
        buf[n] = '\0';
        printf("Server Received: %s (Len=%d bytes) sockfd (%d) \n", buf, n, sockfd);
